package pl.edu.agh.siatka.oscilator.task_4;
...
public class SpringApplet extends JApplet {             // new class inheriting class JApplet
    ...
    public void init(){                 //overwritten method init()
        ...
        this.engine = new SimEngine( 0.5, 4, 0.6, 2, 6, 8, 2, 3, 0, 0);   // creating a new simulation model
    ...
    public void paint(Graphics graph){       //overwritten method paint()
        if(paintCounter > (15 * 1000 / this.stepTime ) ) {            // every 15 seconds the simulation will be repeated
            engine.reset();
            paintCounter = 0;
        }
        paintCounter++;
        ...
        graph.drawString("Time: " + (paintCounter * this.stepTime / 1000 ) + "s", 100, 100);                   // drawing additional information: simulation time
    }
